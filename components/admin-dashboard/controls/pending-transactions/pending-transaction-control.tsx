...
const PendingTransactions: React.FC<{ url: string | undefined; router: AppRouterInstance }> = ({ url, router }) => {
    ...
    return (
        <div className="container mx-auto px-4 py-8 mt-8">
            ...
            {showError ? (
                <div className='text-sm text-center text-red-500 mt-5'>Sorry an error occured while fetching pending transactions.</div>
            ) : (
                <>
                    <div className="flex justify-center items-start flex-wrap gap-x-8 gap-y-6 mt-8">
                        {showLoader ? (
                            ...
                            transactions.map((transaction, index) => <UnitPending key={index} transaction={transaction} url={url} />)
                        )}
                    </div>
                    {showMoreLoader ? (
                        <Loader2 h='h-[1rem]' />
                    ) : (
                        <div className='text-center text-blue-400 mt-3'> <button onClick={() => setpagging(pagging + 1)}>see more..</button></div>
                    )}
                </>
            )
            }
