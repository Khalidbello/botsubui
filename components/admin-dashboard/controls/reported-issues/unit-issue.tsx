...
const UnitIssue: React.FC<UnitIssueProp> = ({ issue, url, router }) => {
    ...
    return (
        <div className="min-w-[18rem] max-w-xl flex-1 bg-white rounded-lg shadow-md p-4">
            <p className="text-lg mb-2"><span className="font-semibold">Description: </span>{issue.description}</p>
            <textarea
            ...
            {showIssueClosed && (<div className="text-blue-500 text-sm px-4"><span className="text-semibold">Issue has been closed</span> {response}</div>)}
            {messageSent && (<div className="text-green-500 text-sm px-4"><span className="text-semibold">Message sent:</span> {response}</div>)}
            {showEmptyError && (<div className="text-red-500 text-sm px-4">Response {`can'nt`} be empty</div>)}
            {showError && (<div className="text-red-500 text-sm px-4">Sorry an error occured...</div>)}

            <div className="flex justify-end gap-x-6 items-center">
                <span><span className="font-semibold">IssueId:</span> {issue.id}</span>
